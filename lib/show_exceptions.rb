...
class ShowExceptions
  ...
  def render_exception(e)
    ...
    template_path = File.join(dir, 'templates', 'rescue.html.erb')
    template = File.read(template_path)

    template_response = ERB.new(template).result(binding)

    Rack::Response.new(template_response, '500', {'Content-Type' => 'text/html'}).finish
  end
