...
-- Not sure why, but it appears that seeding through the sql file won't increment
-- the serial pkey by 1 each time, leading to the two being out of sync
-- this ensures that they will be in sync with each other at the end of sql file seeding
-- and shouldnt be an issue after seeding implementation as mechanism is different
SELECT setval('dragons_id_seq', (SELECT MAX(id) FROM dragons));
